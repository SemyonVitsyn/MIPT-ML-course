...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        pred = np.multiply(self.output, gradOutput)
        self.gradInput = np.subtract(pred, np.multiply(self.output, np.sum(pred, axis=1, keepdims=True)))
        return self.gradInput
...
class LogSoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        softmax = SoftMax()
        self.gradInput = softmax.updateGradInput(input, np.multiply(gradOutput, np.divide(1., softmax.forward(input))))
        return self.gradInput
...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        if self.training:
            ...
            self.output = np.divide(np.subtract(input, batch_mean), np.sqrt(np.add(batch_variance, self.EPS)))
            self.moving_mean = np.add(np.multiply(self.moving_mean, self.alpha), np.multiply(batch_mean, np.subtract(1, self.alpha)))
            self.moving_variance = np.add(np.multiply(self.moving_variance, self.alpha), np.multiply(batch_variance, np.subtract(1, self.alpha)))
        else:
    ...
    def updateGradInput(self, input, gradOutput):
        if self.training:
            denom = np.sqrt(np.add(np.var(input, axis=0), self.EPS))
        else:
            denom = np.sqrt(np.add(self.moving_variance, self.EPS))

        a = np.subtract(np.multiply(gradOutput.shape[0], gradOutput), np.sum(gradOutput, axis=0))
        b = np.multiply(self.output, np.sum(np.multiply(gradOutput, self.output), axis=0))
        self.gradInput = 1 / gradOutput.shape[0] / denom * (a - b)
...
class Dropout(Module):
    ...
    def updateOutput(self, input):
        if self.training:
            self.mask = np.divide(np.random.binomial(1, 1 - self.p, np.shape(input)), np.subtract(1, self.p))
...
class LeakyReLU(Module):
    ...
    def updateOutput(self, input):
        self.output = np.maximum(np.multiply(self.slope, input), input)
        return self.output
    
    def updateGradInput(self, input, gradOutput):
        self.gradInput = np.where(input > 0, gradOutput, np.multiply(self.slope, gradOutput))
        return self.gradInput
...
class ELU(Module):
    ...
    def updateOutput(self, input):
        self.output = np.where(input > 0, input, np.multiply(self.alpha, (np.subtract(np.exp(input), 1))))
        return self.output
    
    def updateGradInput(self, input, gradOutput):
        self.gradInput = np.where(input > 0, gradOutput, np.multiply(gradOutput, np.multiply(self.alpha, (np.exp(input)))))
        return self.gradInput
...
class SoftPlus(Module):
    ...
    def updateOutput(self, input):
        self.output = np.log(np.add(np.exp(input), 1))
        return self.output
    
    def updateGradInput(self, input, gradOutput):
        self.gradInput = np.multiply(gradOutput, np.divide(1, np.add(np.exp(-input), 1)))
        return self.gradInput
...
class ClassNLLCriterion(Criterion):
    ...
    def updateOutput(self, input, target): 
        self.output = -np.mean(np.sum(np.multiply(input, target), axis=1))
        return self.output
...
def adam_optimizer(variables, gradients, config, state):  
    ...
    for current_layer_vars, current_layer_grads in zip(variables, gradients): 
        for current_var, current_grad in zip(current_layer_vars, current_layer_grads):
            ...
            np.add(np.multiply(config['beta2'], var_second_moment), np.multiply((1 - config['beta2']), current_grad**2), out=var_second_moment)
            current_var -= np.divide(np.multiply(lr_t, var_first_moment), np.add(np.sqrt(var_second_moment), config['epsilon']))
